    ...
    return (
        <aside className={`bg-white/3 p-3 h-screen border-r ${collapsed ? 'w-20' : 'w-64'} transition-all duration-200`}>
            <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={()=>setCollapsed(c=>!c)} className="px-2 py-1 text-sm rounded border">{collapsed ? '>' : '<'}</button>
            </div>


            <nav className="space-y-1">
                {items.map(it=> (
                    <Link key={it.to} to={it.to} className={`block rounded px-2 py-2 text-sm ${loc.pathname===it.to ? 'bg-white/10' : 'hover:bg-white/5'}`}>
